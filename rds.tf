...
resource "aws_s3_bucket_logging" "s3_bucket_logging" {
  count                 = var.create_s3_bucket == true ? (var.create_aws_s3_bucket_logging == true ? 1 : 0) : 0
  bucket                = var.bucket
  expected_bucket_owner = data.aws_caller_identity.current.account_id
...
resource "aws_s3_bucket_versioning" "s3_bucket_versioning" {
  count  = var.create_s3_bucket == true ? (var.create_s3_bucket_versioning == true ? 1 : 0) : 0
  bucket = var.bucket
...
resource "aws_s3_bucket_lifecycle_configuration" "bucket_lifecycle_configuration" {
  count                 = var.create_s3_bucket == true ? (var.create_aws_s3_lifecycle_configuration == true ? 1 : 0 ) : 0
  bucket                = var.bucket
  expected_bucket_owner = data.aws_caller_identity.current.account_id
  rule {
...
data "aws_iam_policy_document" "allow_access_from_another_account" {
  statement {
    ...
    condition {
      ...
      values   = ["arn:aws:iam::*:role/OlympusVaultServiceRole", "arn:aws:iam::*:role/antm-sec"]
    }
  }
}
